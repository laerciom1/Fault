...
void Graph::caminhosPossiveis(vector<int> &vetor, int begin, int end){
	...
	if(begin == end){				// Caso encontre o destino, adiciona  
		resposta.push_back(vetor);	// o caminho ao vetor de respostas
	}
...
int main()
{
	...
	vector<int> vetor;
	arq = fopen("saida_5x6_all_mem.txt", "w");
	for(int l = 0; l < rotas; l++){
		...
		// Especificar o arquivo de saida, que contera os caminhos		
		for(unsigned int i = 0; i < resposta.size(); i++){	// Gravando no arquivo as respostas.
			vetor = resposta.at(i);
			fprintf (arq, "%dto%d_%d ", vetor.at(0), vetor.at(vetor.size()-1), i);
