...
public class Grid {
	...
	public int[][] ParametrizavelPadrao(CommunicationMatrix cm, int lookahead) throws IOException{	
		...
		ArrayList<int[]> headsNTails = new ArrayList<int[]>();
		ArrayList<int[]> paths = new ArrayList<int[]>();
		for(int source = 0; source < tasks; source++){ 		//nodes é o numero de nós do grafo
	 								...
	 								if(auxC != destinationC){	// caso a coluna destino seja diferente da coluna atual
			 							...
			 							// e a posição dele no vetor de pontos
 		 								...
 		 								north[1] = pontos[pos][1];
	 										...
	 										// logo a decisão pelo norte será no máximo subir "up" roteadores, onde up foi definido no laço anterior
	 										// sempre testando se a posicao atual nao eh mais vantajosa do que a do proximo salto
 	 									north[0] = auxL;
	 									north[1] = auxC;
	 								...
	 								if(auxL != destinationL){
	 									...
	 									if(auxC > destinationC){
	 										for(int j = 1; j <= left; j++){
		 										if(grid[auxL][auxC-j].isWorking()){
		 											if(Math.sqrt(Math.pow((auxC-j)-destinationC, 2)+Math.pow(auxL-destinationL,2)) < distance){
